...
{
    std::cout << "[Test] Complete POST request processing flow" << std::endl;
    ...
    std::cout << "[Step 1] Parsing POST request with JSON body" << std::endl;
    std::cout << "[Input]\n" << rawRequest << std::endl;
    ...
    std::cout << "[Result] parsed: method=" << request.getMethod() 
              << ", path=" << request.getPath() 
    ...
    try {
        Router::Decision decision = Router::decide(request, config, serverManager);
        std::cout << "[Step 2] routing decision: type=" << decision.type << std::endl;
        
        // 3. Process based on request
        Response response;
        
        if (request.getMethod() == "POST" && request.getPath().find("/api/") == 0) {
            std::cout << "[Step 3] Processing API POST request" << std::endl;
        ...
        std::cout << "[Result] POST response generated (" << serialized.length() << " bytes)" << std::endl;
...
{
    std::cout << "[Test] Error handling throughout the flow" << std::endl;
    ...
    std::cout << "[Step 1] Testing malformed request handling" << std::endl;
    std::cout << "[Input] " << malformedRequest << std::endl;
    
    parser.feed(malformedRequest);
    Request request = parser.getRequest();
    
    std::cout << "[Result] parser error=" << (request.getError() ? "true" : "false");
    if (request.getError()) {
    ...
    if (request.getError()) {
        std::cout << "[Step 2] Generating error response for malformed request" << std::endl;
        ...
        std::cout << "[Result] error response generated: status=" << errorResponse.getStatusCode() << std::endl;
...
{
    std::cout << "[Test] Request with cookies processing flow" << std::endl;
    ...
    std::cout << "[Step 1] Parsing request with cookies" << std::endl;
    std::cout << "[Input]\n" << rawRequest << std::endl;
    ...
    std::cout << "[Result] cookies header: " << request.getHeader("cookie") << std::endl;
    
    // 2. Process request and set response cookies
    std::cout << "[Step 2] Processing authenticated request" << std::endl;
    Response response;
    
    std::string cookieHeader = request.getHeader("cookie");
    if (!cookieHeader.empty() && cookieHeader.find("sessionId=") != std::string::npos) {
        std::cout << "[Processing] Valid session found" << std::endl;
        response.setStatus(200, "OK");
        response.setHeader("Content-Type", "text/html");
        ...
        std::cout << "[Processing] No valid session" << std::endl;
        response.setStatus(401, "Unauthorized");
        response.setBody("Please log in");
    }
    
    std::string serialized = response.serialize();
    std::cout << "[Result] response with session handling (" << serialized.length() << " bytes)" << std::endl;
...
{
    std::cout << "[Test] Chunked response generation flow" << std::endl;
    ...
    std::cout << "[Input] Large content (" << largeContent.length() << " bytes)" << std::endl;
    std::cout << "[Expect] Chunked encoding response" << std::endl;
    
    std::string serialized = response.serialize();
    std::cout << "[Result] chunked=" << (response.isChunked() ? "true" : "false") << std::endl;
    std::cout << "[Result] has Transfer-Encoding=" << (serialized.find("Transfer-Encoding: chunked") != std::string::npos ? "true" : "false") << std::endl;
    std::cout << "[Result] no Content-Length=" << (serialized.find("Content-Length:") == std::string::npos ? "true" : "false") << std::endl;
